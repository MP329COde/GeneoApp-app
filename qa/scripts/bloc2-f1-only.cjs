...
async function createPerson(page, prenom, nom, sexe) {
  ...
  try { await sexeSelect.selectOption(val); } catch (e) {}
...
async function goFamillesScreen(page) { await closeAnyModal(page); await page.getByText('Familles', { exact: true }).first().click({ timeout: 8000 }); await page.waitForTimeout(400); }
...
(async () => {
  ...
  for (const p of data.personnes) {
    try { await createPerson(page, p.prenom, p.nom, p.sexe); note('personne-creee', p.prenom + ' ' + p.nom); }
    catch (e) { note('bug-personne', { p: p.prenom + ' ' + p.nom, err: String(e).slice(0,150) }); }
  }
  for (const u of data.unions) {
    const nameA = data.byId[u.a], nameB = data.byId[u.b];
    try { await createUnion(page, nameA, nameB); note('union-creee', { nameA, nameB }); }
    catch (e) { note('bug-union', { nameA, nameB, err: String(e).slice(0,150) }); }
  }
  for (const pa of data.parentages) {
    const enfant = data.byId[pa.enfant], parent = data.byId[pa.parent];
    try { await addParent(page, enfant, parent); note('parent-ajoute', { enfant, parent }); }
    catch (e) { note('bug-parentage', { enfant, parent, err: String(e).slice(0,150) }); }
  }
  await page.screenshot({ path: path.join(__dirname, '..', 'reports', 'screenshots-bloc2', 'f1-reprise-final.png') });
  await browser.close();
})();
