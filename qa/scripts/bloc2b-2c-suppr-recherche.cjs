...
async function openTree(page, exactName) {
  ...
  if (await btn.count() > 0) { await btn.click({ timeout: 8000 }); await page.waitForTimeout(600); }
...
async function selectPerson(page, name) {
  ...
  if (await filter.isVisible().catch(() => false)) { await filter.fill(name); await page.waitForTimeout(400); }
...
(async () => {
  ...
  page.on('dialog', async (d) => { note('dialog-confirmation', d.message()); await d.accept(); });
  ...
  try {
    ...
    const confirmBtn = page.getByRole('button', { name: /Confirmer|Oui|Supprimer définitivement/i }).first();
    if (await confirmBtn.isVisible({ timeout: 1500 }).catch(() => false)) { await confirmBtn.click(); await page.waitForTimeout(400); }
    note('suppression-personne-feuille', 'Gustave Dupont');
    await shot(page, '21-apres-suppr-feuille.png');
  } catch (e) { note('bug-suppr-feuille', String(e).slice(0, 250)); await shot(page, '21-bug-suppr-feuille.png'); }
  ...
  try {
    ...
    await page.getByRole('button', { name: /^Rechercher$/ }).first().click({ timeout: 5000 });
  ...
  } catch (e) { note('bug-verif-recherche-feuille', String(e).slice(0, 200)); }
  ...
  } catch (e) { note('bug-suppr-centrale', String(e).slice(0, 250)); await shot(page, '24-bug-suppr-centrale.png'); }
  ...
  try {
    ...
    note('verif-orphelin-antoine', 'capture prise, voir si Parents pointe vers un id inexistant ou proprement retiré');
  } catch (e) { note('bug-verif-orphelin', String(e).slice(0, 200)); }
  ...
  try {
    ...
    const retirerBtn = page.getByRole('button', { name: /Retirer|Dissoudre|Supprimer l.union/i }).first();
    if (await retirerBtn.count() > 0) {
      await retirerBtn.click({ timeout: 5000 });
      await page.waitForTimeout(400);
      note('suppression-union', 'Antoine/Catherine Dupont');
    } else {
      note('bouton-suppression-union-introuvable', 'aucun bouton Retirer/Dissoudre trouvé sur écran Familles pour une union');
    }
    await shot(page, '27-apres-suppr-union.png');
  } catch (e) { note('bug-suppr-union', String(e).slice(0, 250)); }

  // 4) Supprimer un arbre entier : sacrifier "QA - Famille Dupont-Bernard"
  try {
    await goArbres(page);
    await shot(page, '28-avant-suppr-arbre.png');
    const supprArbreBtn = page.getByRole('button', { name: /^Supprimer QA - Famille Dupont-Bernard$/ }).first();
  ...
  } catch (e) { note('bug-suppr-arbre', String(e).slice(0, 250)); }
  ...
  } catch (e) { note('bug-verif-arbre-supprime', String(e).slice(0, 200)); }

  fs.writeFileSync(LOG, JSON.stringify(log, null, 2));

  // ===== BLOC 2C : recherche sur les 2 arbres restants =====
  const casRecherche = [
    { arbre: "QA - Famille La Tour-d'Auvergne (reprise)", q: 'Dupond', desc: 'orthographe proche (Eléonore Dupond)' },
    { arbre: "QA - Famille La Tour-d'Auvergne (reprise)", q: 'dupond', desc: 'casse différente' },
    { arbre: "QA - Famille La Tour-d'Auvergne (reprise)", q: 'Eleonore', desc: 'sans accent (Eléonore)' },
    { arbre: "QA - Famille La Tour-d'Auvergne (reprise)", q: "N'Diaye", desc: 'apostrophe' },
    { arbre: "QA - Famille La Tour-d'Auvergne (reprise)", q: 'xyzintrouvable', desc: 'résultat vide' },
  ...
  for (const c of casRecherche) {
    if (c.arbre !== currentArbre) { await openTree(page, c.arbre); currentArbre = c.arbre; }
    try {
      await menu(page, 'Recherche');
      const champ = page.getByPlaceholder(/Nom, lieu, source/i).first();
      ...
      note('recherche', { q: c.q, desc: c.desc, resultatsApprox: nbResultatsMatch ? nbResultatsMatch[1] : 'inconnu' });
      await shot(page, 'recherche-' + c.q.replace(/[^a-z0-9]+/gi, '-') + '.png');
    } catch (e) { note('bug-recherche', { q: c.q, err: String(e).slice(0, 200) }); }
  }
