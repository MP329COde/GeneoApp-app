...
(async () => {
  ...
  const switchBtn = page
    .getByRole('button', { name: /^Ouvrir QA - Famille Muller-Ndiaye$/ })
    .first();
  if ((await switchBtn.count()) > 0) {
    ...
    note('deja-sur-un-autre-arbre', true);
  }
  ...
  if ((await btn.count()) === 0) {
    note('bouton-supprimer-introuvable', true);
  } else {
  ...
  await page
    .getByText('Recherche', { exact: true })
    .first()
    .click({ timeout: 8000 })
    .catch(() => {});
