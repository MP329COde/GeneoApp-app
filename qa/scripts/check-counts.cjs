...
(async () => {
  ...
  for (const name of ['QA - Famille Muller-Ndiaye', 'QA - Famille Dupont-Bernard']) {
    ...
    if (await btn.count() === 0) { note('introuvable', name); continue; }
    ...
    await page.waitForTimeout(300);
    await page.screenshot({ path: path.join(__dirname, '..', 'reports', 'screenshots-bloc2', 'check-' + name.replace(/[^a-z0-9]+/gi, '-') + '.png') });
    await page.getByText('Arbres', { exact: true }).first().click();
    await page.waitForTimeout(500);
  }
  await browser.close();
