...
function note(step, info) {
  console.log('[' + step + ']', JSON.stringify(info).slice(0, 400));
...
(async () => {
  ...
  const familles = [
    {
      file: 'famille-1-de-la-tour-dauvergne.json',
      arbre: "QA - Famille La Tour-d'Auvergne",
      unions: true,
      parentages: true,
    },
    {
      file: 'famille-2-mueller-ndiaye.json',
      arbre: 'QA - Famille Muller-Ndiaye',
      unions: false,
      parentages: true,
    },
    {
      file: 'famille-3-dupont-bernard.json',
      arbre: 'QA - Famille Dupont-Bernard',
      unions: false,
      parentages: true,
    },
  ];
  ...
  for (const f of familles) {
    ...
    const arbreKey = f.arbre.includes('Muller')
      ? 'Muller-Ndiaye'
      : f.arbre.includes('Dupont')
        ? 'Dupont-Bernard'
        : 'Autre';
    data.parentages = data.parentages.filter(
      (pa) => !done.has(arbreKey + '|' + data.byId[pa.enfant] + '|' + data.byId[pa.parent]),
    );
    for (const u of data.unions) {
      ...
      try {
        ...
        note('union-creee', { arbre: f.arbre, nameA, nameB });
      } catch (e) {
        note('bug-union', { arbre: f.arbre, nameA, nameB, err: String(e).slice(0, 200) });
      }
    }
    for (const pa of data.parentages) {
      ...
      try {
        ...
        note('parent-ajoute', { arbre: f.arbre, enfant, parent });
      } catch (e) {
        note('bug-parentage', { arbre: f.arbre, enfant, parent, err: String(e).slice(0, 200) });
      }
    }
