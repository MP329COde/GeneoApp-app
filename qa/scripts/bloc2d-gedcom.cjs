...
(async () => {
  ...
  for (const c of cas) {
    try {
      ...
      if ((await apercuBtn.count()) > 0) {
        await apercuBtn
          .click({ timeout: 5000 })
          .catch((e) => note('bug-clic-apercu', { c: c.label, err: String(e).slice(0, 150) }));
      }
      await page.waitForTimeout(1000);
      ...
      if ((await importBtn.count()) > 0) {
        await importBtn
          .click({ timeout: 8000 })
          .catch((e) => note('bug-clic-importer', { c: c.label, err: String(e).slice(0, 150) }));
      }
      await page.waitForTimeout(1500);
      await shot(page, '41-import-' + c.label + '.png');
      note('import-tente', c.label);
    } catch (e) {
      note('bug-import', { label: c.label, err: String(e).slice(0, 300) });
      await shot(page, '41-bug-import-' + c.label + '.png');
    }
  }
  ...
  try {
    ...
    const apercuBtn = page.getByRole('button', { name: /^Aperçu$/i }).first();
    if ((await apercuBtn.count()) > 0) {
      await apercuBtn.click({ timeout: 5000 }).catch(() => {});
    }
    await page.waitForTimeout(1000);
    const importBtn = page.getByRole('button', { name: /^Importer/i }).first();
    if ((await importBtn.count()) > 0) {
    ...
    for (let i = 0; i < 30; i++) {
      ...
      const bodyText = await page
        ...
        .textContent()
        .catch(() => '');
      if (/terminé|importé|erreur|échec/i.test(bodyText)) {
        done = true;
        break;
      }
    }
    ...
    note('bug-import-volumineux', String(e).slice(0, 300));
  }
