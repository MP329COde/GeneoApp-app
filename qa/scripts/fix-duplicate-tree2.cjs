...
(async () => {
  ...
  page.on('dialog', async (d) => { note('dialog', d.message()); await d.accept(); });
    ...
    for (const card of cards) {
      ...
      if (text.includes("QA - Famille La Tour-d'Auvergne") && text.includes('0 personne(s)')) {
        ...
        if (await supBtn.count() > 0) {
          ...
          await supBtn.first().click({ timeout: 5000 }).catch((e) => note('bug-suppr-click', String(e).slice(0, 200)));
          await page.waitForTimeout(600);
          removed = true;
          break;
        } else {
  ...
  await page.screenshot({ path: path.join(__dirname, '..', 'reports', 'screenshots-bloc2', 'apres-suppression-doublons.png') });
  const html = await page.content();
  require('fs').writeFileSync(path.join(__dirname, '..', 'reports', 'apres-suppression-doublons.html'), html);
  await browser.close();
})();
