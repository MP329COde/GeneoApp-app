...
const log = [];
function note(s, i) {
  console.log('[' + s + ']', JSON.stringify(i).slice(0, 500));
  ...
  for (const img of images) {
    try {
      ...
      const finalText = await page
        ...
        .catch(() => '');
      fs.writeFileSync(
        path.join(__dirname, '..', 'reports', 'ocr-body-' + img.replace('.png', '') + '.txt'),
        finalText,
      );
      note('ocr-traite', img);
    } catch (e) {
      note('bug-ocr', { img, err: String(e).slice(0, 300) });
      await shot(page, '51-bug-ocr-' + img.replace('.png', '') + '.png');
    }
    // réinitialiser pour le prochain fichier si besoin (recharger l'écran)
    await page
      .getByText('Déchiffrer & identifier', { exact: true })
      .first()
      .click({ timeout: 8000 })
      .catch(() => {});
