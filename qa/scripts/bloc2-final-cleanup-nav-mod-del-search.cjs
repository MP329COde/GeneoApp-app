...
(async () => {
  ...
  try {
    ...
    note('bug-onglets-vue-arbre', String(e).slice(0, 200));
  }
  try {
    ...
    note('bug-zoom-recentrer', String(e).slice(0, 200));
  }
  ...
  try {
    ...
    if (await filter.isVisible().catch(() => false)) {
      await filter.fill('Suzanne');
      await page.waitForTimeout(300);
    }
    await page
      .getByText("Suzanne de La Tour-d'Auvergne", { exact: true })
      .last()
      .click({ timeout: 5000 });
    await page.waitForTimeout(400);
    await menu(page, 'Personne');
    await page
      .getByRole('button', { name: /^Identité$/ })
      .click({ timeout: 3000 })
      .catch(() => {});
    const champSurnom = page.getByLabel(/Surnom/i).first();
    if ((await champSurnom.count()) > 0) {
      await champSurnom.fill('Suzon (modifiée QA)').catch(() => {});
    }
    await page
      .getByRole('button', { name: /Enregistrer/i })
      .first()
      .click({ timeout: 5000 })
      .catch((e) => note('bug-enregistrer-modif', String(e).slice(0, 150)));
    note('modification-personne', 'tentee-sur-Suzanne');
    await shot(page, '15-personne-modifiee.png');
  } catch (e) {
    note('bug-modification-personne', String(e).slice(0, 250));
  }
